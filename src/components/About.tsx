    ...
    <section className="py-16 md:py-24 bg-white" id="about-us">
      <div className="max-w-7xl mx-auto px-4 sm:px-6">
        ...
        <div className="text-center max-w-3xl mx-auto mb-16">
          ...
          <h2 className="font-display text-3xl sm:text-4xl md:text-5xl font-medium tracking-tight text-slate-900 mt-3">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center mb-20">
          ...
          {/* Image block on mobile first, desktop right */}
          ...
          <div className="lg:col-span-6 order-last lg:order-first space-y-6 text-left">
            <h3 className="font-display text-2xl font-medium text-slate-800">
        ...
        {/* Beautiful statistics cards wrapper */}
        <div className="flex gap-4 overflow-x-auto pb-4 pt-6 snap-x snap-mandatory [&::-webkit-scrollbar]:h-2 [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300">
          {stats.map((item, idx) => (
            <div 
              key={idx} 
              className="min-w-[260px] flex-0 snap-start bg-gradient-to-b from-[#fffaf4] to-[#f2f8ff] p-6 rounded-2xl border border-slate-100/80 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 text-center group"
              id={`stat-card-${idx}`}
            >
              <div className="font-display text-4xl font-semibold text-logo-blue-700 mb-2 group-hover:scale-105 transition-transform">
                {item.value}
              </div>
              <h4 className="text-slate-800 font-medium text-sm tracking-tight mb-1">
                {item.label}
              </h4>
